...
namespace TheraBytes.BetterUi
{
    ...
    public class LocationAnimations : MonoBehaviour
    {
        ...
        private RectTransformData GetLocationTransformFallbackCurrent(string name)
        {
            ...
            if(loc == null)
            {
                return new RectTransformData(RectTransform) { SaveRotationAsEuler = true };
            }
            else
            {
                var result = loc.CurrentTransformData;
        ...
        public void OnValidate()
        {
            ...
            for (int i = 0; i < locations.Count; i++)
            {
                ...
                bool isUnInitialized =
                       cur.Scale == Vector3.zero
                    && cur.Rotation.eulerAngles == Vector3.zero
                    && cur.AnchoredPosition == Vector2.zero
                    && cur.AnchorMin == Vector2.zero
                    && cur.AnchorMax == Vector2.zero
                    && cur.Pivot == Vector2.zero
                    && cur.SizeDelta == Vector2.zero
                    && cur.LocalPosition == Vector3.zero;
